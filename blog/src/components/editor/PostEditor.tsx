...
export default () => {
  ...
  async function publish() {
    ...
    try {
      const { data: post } = await axios.post(process.env.NEXT_PUBLIC_BACKEND_URL + 'blog/posts/', {
        title: title,
      ...
      // console.log('tezts uploaded')
      if (cover) {
        const formData = new FormData()
        formData.append('cover', cover)
        const { data } = await axios.put(process.env.NEXT_PUBLIC_BACKEND_URL + 'blog/posts/'+`${post.id}/cover/`, formData, {
          headers: {
  ...
  async function save_as_draft() {
    ...
    try {
      const { data: post } = await axios.post(process.env.NEXT_PUBLIC_BACKEND_URL + 'blog/drafts/', {
        title: title,
        content: editor?.getHTML(),
        tags: selectedTags,
        status:'Draft'
      ...
      if (cover) {
        const formData = new FormData()
        formData.append('cover', cover)
        const { data } = await axios.put(process.env.NEXT_PUBLIC_BACKEND_URL + 'blog/drafts/'+`${post.id}/cover/`, formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
            Authorization: `Bearer ${(session.data as any).access_token}`
          }
  ...
  return (
    <>
      ...
      <div className="p-3">
        ...
        <Button onClick={()=>save_as_draft()} size='sm'>
          Save as draft
        </Button>
